...
export function AppSidebar() {
  return (
    <Sidebar className="border-r border-border bg-background/50 backdrop-blur-xl">
      ...
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupLabel className="text-xs text-t2 uppercase tracking-wider">Intelligence</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {menuItems.map((item) => (
        ...
        <SidebarGroup>
          <SidebarGroupLabel className="text-xs text-t2 uppercase tracking-wider">User</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {userItems.map((item) => (
                <SidebarMenuItem key={item.title}>
      ...
      <SidebarFooter className="p-4 border-t border-border">
        <div className="flex items-center gap-3">
          <div className="h-8 w-8 rounded-full bg-primary flex items-center justify-center text-t1 font-bold text-sm">
            SP
          </div>
          <div className="flex flex-col">
            <span className="text-sm font-medium">Officer Name</span>
            <span className="text-xs text-t2">Superintendent</span>
          </div>
        </div>
      </SidebarFooter>
    </Sidebar>
