...
export default function LandingPage() {
  ...
  const [chartHeights, setChartHeights] = useState([72, 48, 88, 56, 94, 40, 78, 62, 50, 86, 44, 68])
  const [firsToday, setFirsToday] = useState(142)
  const [activeCases, setActiveCases] = useState(3892)
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setChartHeights((prev) => prev.map((h) => Math.max(20, Math.min(100, h + (Math.random() * 20 - 10)))))
      if (Math.random() > 0.5) setFirsToday((prev) => prev + 1)
      if (Math.random() > 0.3) setActiveCases((prev) => prev + (Math.random() > 0.5 ? 1 : -1))
    }, 2500)
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[var(--color-background)]" style={{ color: P.text1, fontFamily: "var(--font-sans)" }}>

      {/* ═══ NAV ═══ */}
      <nav className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled ? "py-3" : "py-5"}`}
        style={{ background: scrolled ? P.panel : "transparent", backdropFilter: scrolled ? "blur(20px) saturate(1.4)" : "none", borderBottom: scrolled ? `1px solid ${P.border}` : "none" }}>
        <div className="max-w-[1320px] mx-auto px-6 lg:px-10 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5 group">
            <div className="w-9 h-9 rounded-[20px] flex items-center justify-center bg-gradient-to-br from-[#3B82F6] to-[#60A5FA] group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(59,130,246,0.4)] transition-all">
              <Shield className="w-4 h-4 text-[var(--color-text1)]" />
            </div>
            <span className="text-[var(--color-text1)] font-bold tracking-[0.14em] text-sm uppercase">Kaaval</span>
          </Link>
          <div className="hidden md:flex items-center gap-8 text-[13px] font-medium text-[#A1A1AA]">
            <button onClick={() => scrollTo("features")} className="hover:text-[var(--color-text1)] transition-colors">Features</button>
            <button onClick={() => scrollTo("modules")} className="hover:text-[var(--color-text1)] transition-colors">Modules</button>
            <Link href="/assistant" className="hover:text-[var(--color-text1)] transition-colors">AI Assistant</Link>
          </div>
          <div className="flex items-center gap-3">
            <ThemeToggle />
            <Link href="/login" className="hidden sm:flex text-[13px] font-medium text-[var(--color-text2)] hover:text-[var(--color-text1)] transition-colors">Login</Link>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-6 pt-28 pb-20 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none select-none">
          <div className="absolute -top-32 left-1/4 w-[700px] h-[700px] rounded-full blur-[200px]" style={{ background: "rgba(59,130,246,0.06)", animation: "ambient-drift 18s ease-in-out infinite" }} />
          <div className="absolute -bottom-40 right-1/4 w-[600px] h-[600px] rounded-full blur-[180px]" style={{ background: "rgba(139,92,246,0.04)", animation: "ambient-drift 22s ease-in-out infinite reverse" }} />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full blur-[220px]" style={{ background: "rgba(59,130,246,0.03)" }} />
        </div>
        ...
        <div className={`relative z-10 text-center max-w-4xl mx-auto transition-all duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)] ${mounted ? "opacity-100 translate-y-0 blur-0" : "opacity-0 translate-y-12 blur-[6px]"}`}>
          ...
          <h1 className="text-[clamp(2.8rem,7vw,5.5rem)] font-extrabold tracking-[-0.04em] leading-[1.05] text-[var(--color-text1)] mb-6">
            Protecting Karnataka
            <br />with{" "}
            <span className="relative inline-block">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#3B82F6] via-[#60A5FA] to-[#8B5CF6]" style={{ backgroundSize: "200% auto", animation: "gradient-shift 4s ease infinite" }}>
                AI Intelligence
              </span>
              <span className="absolute -bottom-1 left-0 w-full h-[3px] rounded-full bg-gradient-to-r from-[#3B82F6] to-[#8B5CF6] opacity-50" />
            </span>
          </h1>

          <p className="text-[clamp(1rem,2vw,1.25rem)] font-light leading-relaxed max-w-2xl mx-auto mb-10 text-[#A1A1AA]">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <Link href="/dashboard" className="group h-14 px-8 rounded-[28px] text-white text-[15px] font-bold flex items-center gap-2.5 transition-all hover:-translate-y-1 bg-gradient-to-r from-[#3B82F6] to-[#60A5FA] hover:shadow-[0_8px_40px_rgba(59,130,246,0.35)]">
              Launch Command Center
              <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-0.5" />
            </Link>
            <Link href="/assistant" className="h-14 px-8 rounded-[28px] text-[15px] font-medium flex items-center gap-2.5 transition-all hover:bg-white/[0.06] glass-panel">
        ...
        <button onClick={() => scrollTo("stats")} className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-[#A1A1AA] hover:text-[var(--color-text1)] transition-colors z-10">
      ...
      {/* ═══ STATS ═══ */}
      <section id="stats" style={{ borderTop: `1px solid rgba(255,255,255,0.06)`, borderBottom: `1px solid rgba(255,255,255,0.06)`, background: "rgba(59,130,246,0.02)" }}>
          ...
          {[
            ...
            <R key={s.label} delay={i * 100}>
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-[20px] flex items-center justify-center shrink-0" style={{ background: `${s.color}10` }}>
                  <s.icon className="w-5 h-5" style={{ color: s.color }} />
                </div>
                <div>
                  <div className="text-2xl font-bold text-[var(--color-text1)] tracking-tight">{s.value}</div>
        ...
        <div className="max-w-[1320px] mx-auto">
          <R>
            <div className="max-w-2xl mb-20">
              ...
              <h2 className="text-[clamp(1.8rem,4vw,3rem)] font-bold text-[var(--color-text1)] tracking-tight leading-tight mb-5">
          ...
          {/* Dashboard Preview */}
          <R>
            <div className="rounded-[28px] p-[1px] mb-20" style={{ background: "linear-gradient(135deg, rgba(59,130,246,0.2), rgba(139,92,246,0.15), rgba(255,255,255,0.03))" }}>
              <div className="rounded-[15px] p-6 lg:p-8" style={{ background: "#111113" }}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-[20px] flex items-center justify-center" style={{ background: `${P.coral}12` }}>
                      <BarChart3 className="w-5 h-5" style={{ color: P.coral }} />
                    </div>
                    <div>
                      <h3 className="text-[15px] font-semibold text-[var(--color-text1)]">Statewide Analytics</h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                  {[
                    { t: "FIRs Filed Today", v: firsToday.toLocaleString(), c: "+12%", icon: FileText, col: P.coral },
                    { t: "Active Cases", v: activeCases.toLocaleString(), c: "14 critical", icon: Activity, col: P.amber },
                    ...
                    <div key={m.t} className="p-5 rounded-[20px] group hover:-translate-y-0.5 transition-all" style={{ background: "rgba(255,255,255,0.02)", border: `1px solid rgba(255,255,255,0.04)` }}>
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-[10px] font-semibold uppercase tracking-wider text-[#A1A1AA]">{m.t}</span>
                      ...
                      <div className="text-[1.6rem] font-bold text-[var(--color-text1)] mb-1 tracking-tight">{m.v}</div>
                ...
                <div className="p-5 rounded-[20px]" style={{ background: "rgba(255,255,255,0.02)", border: `1px solid rgba(255,255,255,0.04)` }}>
                  <div className="flex items-center justify-between mb-6">
                    <span className="text-[13px] font-semibold text-[var(--color-text1)]">District Crime Distribution</span>
                  ...
                  <div className="flex items-end gap-[6px] h-36">
                    {chartHeights.map((h, i) => (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                        ...
                        <span className="text-[8px] font-mono text-[#71717A]">{["BLR","MYS","MNG","HBL","BLG","DVG","TUM","KLB","SHM","GBG","UDN","CHK"][i]}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div id="modules">
            <R><div className="max-w-xl mb-14">
              ...
              <h2 className="text-[clamp(1.5rem,3.5vw,2.4rem)] font-bold text-[var(--color-text1)] tracking-tight leading-tight">Purpose-Built Tools for Every Operation</h2>
            </div></R>
              ...
              {[
                ...
                <R key={m.title} delay={i * 80}>
                  <Link href={m.href} className="group block h-full">
                    <div className="glass-panel h-full p-7 rounded-[28px] transition-all duration-300 hover:-translate-y-1 flex flex-col relative overflow-hidden">
                      ...
                      <h3 className="relative text-[16px] font-semibold text-[var(--color-text1)] mb-2 group-hover:text-[#3B82F6] transition-colors duration-300">{m.title}</h3>
      ...
      <section className="relative py-28 px-6 overflow-hidden" style={{ borderTop: `1px solid rgba(255,255,255,0.04)` }}>
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full blur-[200px]" style={{ background: "rgba(59,130,246,0.04)" }} />
        </div>
        <R>
          <div className="relative z-10 max-w-2xl mx-auto text-center space-y-7">
            <h2 className="text-[clamp(1.8rem,4vw,3rem)] font-bold text-[var(--color-text1)] tracking-tight leading-tight">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 pt-2">
              <Link href="/dashboard" className="group h-14 px-8 rounded-[28px] text-[var(--color-text1)] text-[15px] font-bold flex items-center gap-2.5 transition-all hover:-translate-y-1 bg-gradient-to-r from-[#3B82F6] to-[#60A5FA] hover:shadow-[0_8px_40px_rgba(59,130,246,0.35)]">
        ...
        <div className="max-w-[1320px] mx-auto flex flex-col md:flex-row items-center justify-between gap-5">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center bg-gradient-to-br from-[#3B82F6] to-[#60A5FA]">
              <Shield className="w-3.5 h-3.5 text-[var(--color-text1)]" />
            </div>
            <span className="text-[12px] font-bold tracking-[0.12em] text-[#A1A1AA] uppercase">Kaaval</span>
          </div>
          <p className="text-[11px] font-mono tracking-wide text-[#71717A]">State Crime Records Bureau (SCRB) · Government of Karnataka · © 2026</p>
