...
export default function AssistantPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch("https://kaaval-backend-50044365217.development.catalystappsail.in/api/intelligence/query", {
        ...
        <Reveal>
          <div className="flex items-end justify-between gap-4 pb-5 mb-5" style={{ borderBottom: `1px solid ${P.border}` }}>
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-t1 leading-tight">Neural Interface</h1>
                  ...
                  <div className={`flex items-start gap-3 sm:gap-4 max-w-[90%] sm:max-w-[80%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    <div className={`flex flex-col gap-2 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                      ...
                      <div className={`px-5 py-4 text-[13.5px] leading-relaxed shadow-lg
                        ...
                        ${msg.role === 'user' 
                          ? 'text-t1 rounded-[20px] rounded-tr-[4px]' 
            ...
            <div className="p-4 sm:p-5 shrink-0" style={{ borderTop: `1px solid ${P.border}`, background: "transparent" }}>
              <div className="relative flex items-center">
                <input 
                  ...
                  className="w-full rounded-[28px] pl-5 pr-28 py-4 focus:outline-none transition-all text-[13px] font-medium placeholder:text-t3"
                ...
                <div className="absolute right-2 flex items-center gap-1.5">
                  ...
                  >
                    <Mic className={`w-5 h-5 transition-transform ${isListening ? "animate-pulse scale-110" : "group-hover:text-t1"}`} />
                  </button>
                  <button 
                    onClick={handleSend}
            ...
            <div className="space-y-4 flex-1 overflow-y-auto pr-1">
              {[
                ...
                <div key={i} className="flex items-center justify-between group">
                  <span className="text-[12px] font-medium text-t1 transition-colors group-hover:text-[#3B82F6]">{node.name}</span>
