      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 h-full">
        ...
        <Reveal>
          <div className="flex items-end justify-between gap-4 pb-5 mb-5" style={{ borderBottom: `1px solid ${P.border}` }}>
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-[28px] relative group overflow-hidden" style={{ background: `${P.violet}15`, border: `1px solid ${P.violet}30` }}>
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-[var(--color-text1)] leading-tight">Neural Interface</h1>
        ...
        <Reveal delay={100} className="flex-1 min-h-0">
          <div className="glass-panel flex flex-col overflow-hidden h-full rounded-3xl relative border" style={{ borderColor: `${P.border}`, background: P.surface }}>
              ...
              {messages.map((msg, i) => (
                <Reveal key={msg.id} delay={i * 50} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`flex items-start gap-3 sm:gap-4 max-w-[90%] sm:max-w-[80%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    {/* Avatar */}
                    {msg.role !== 'user' ? (
                      <div className="w-10 h-10 rounded-[20px] flex items-center justify-center shrink-0 border mt-1" style={{ background: `${P.violet}15`, borderColor: `${P.violet}40`, color: P.violet, boxShadow: `0 0 12px ${P.violet}20` }}>
                        <ShieldAlert className="w-5 h-5" />
                      </div>
                    ) : (
                      <div className="w-10 h-10 rounded-[20px] flex items-center justify-center shrink-0 border mt-1" style={{ background: "rgba(255,255,255,0.05)", borderColor: P.border, color: P.text2 }}>
                    ...
                    <div className={`flex flex-col gap-2 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                      ...
                      <div className={`px-5 py-4 text-[13.5px] leading-relaxed shadow-lg
                        ...
                        ${msg.role === 'user' 
                          ? 'text-[var(--color-text1)] rounded-[20px] rounded-tr-[4px]' 
                        ...
                        style={msg.role === 'user' 
                          ...
                          : { background: "#111113", borderColor: "rgba(255,255,255,0.08)" }
                        }
                      >
              ...
              {isLoading && (
                <div className="flex justify-start">
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-[20px] flex items-center justify-center shrink-0 border" style={{ background: `${P.violet}15`, borderColor: `${P.violet}40`, color: P.violet }}>
                      <ShieldAlert className="w-5 h-5 animate-pulse" />
                    </div>
                    <div className="px-5 py-4 rounded-[20px] rounded-tl-[4px] flex items-center gap-3 border shadow-lg" style={{ background: P.panel, borderColor: P.border }}>
            ...
            {/* Input Area */}
            <div className="p-4 sm:p-5 shrink-0" style={{ borderTop: `1px solid ${P.border}`, background: "transparent" }}>
              <div className="relative flex items-center">
                <input 
                  ...
                  className="w-full rounded-[28px] pl-5 pr-28 py-4 focus:outline-none transition-all text-[13px] font-medium placeholder:text-[var(--color-text3)]"
                ...
                <div className="absolute right-2 flex items-center gap-1.5">
                  ...
                  >
                    <Mic className={`w-5 h-5 transition-transform ${isListening ? "animate-pulse scale-110" : "group-hover:text-[var(--color-text1)]"}`} />
                  </button>
                  <button 
                    onClick={handleSend}
                    disabled={isLoading || !input.trim()}
                    className="p-2.5 rounded-[20px] text-[var(--color-text1)] transition-all disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105"
      ...
      <div className="hidden lg:flex flex-col w-[320px] shrink-0 gap-5 min-h-0">
            ...
            <div className="space-y-4 flex-1 overflow-y-auto pr-1">
              {[
                ...
                <div key={i} className="flex items-center justify-between group">
                  <span className="text-[12px] font-medium text-[var(--color-text1)] transition-colors group-hover:text-[#3B82F6]">{node.name}</span>
        ...
        <Reveal delay={200} className="flex-1 flex flex-col min-h-0">
          <div className="glass-panel flex-1 rounded-3xl p-5 flex flex-col border relative overflow-hidden group" style={{ borderColor: P.border, background: P.surface }}>
            ...
            <div className="flex-1 relative rounded-[28px] overflow-hidden bg-[#09090B] border" style={{ borderColor: P.border }}>
              {/* Radar rings */}
