...
export default function TimelineBuilderPage() {
  ...
  return (
    <div className="max-w-[1320px] mx-auto space-y-6 pb-20">
      {notification && (
        <div className="fixed top-24 right-6 z-50 px-5 py-3 rounded-[28px] text-[13px] font-bold flex items-center gap-2 animate-in fade-in slide-in-from-top-4" style={{ background: P.surface, backdropFilter: "blur(24px)", border: `1px solid ${P.green}40`, color: P.green, boxShadow: `0 8px 32px ${P.green}20` }}>
      ...
      <Reveal>
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6" style={{ borderBottom: `1px solid ${P.border}` }}>
          <div className="flex items-start gap-4">
            <div className="p-3 rounded-[28px]" style={{ background: `${P.blue}15`, color: P.blue, border: `1px solid ${P.blue}30` }}>
              <GitCommit className="w-6 h-6" />
            </div>
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-[var(--color-text1)] leading-tight">Case Management & Timelines</h1>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 p-1.5 rounded-[28px] glass-panel">
              <span className="text-[12px] font-semibold px-2" style={{ color: P.text2 }}>Active Case:</span>
              <select value={selectedCaseId} onChange={(e) => setSelectedCaseId(e.target.value)} className="bg-transparent font-mono text-[12px] font-bold px-2 py-1.5 rounded-[20px] focus:outline-none cursor-pointer" style={{ color: P.blue }}>
                {cases.map((c) => <option key={c.case_id} value={c.case_id} style={{ background: "#09090B", color: P.text1 }}>{c.case_id} - {c.title.slice(0, 30)}...</option>)}
              </select>
            </div>
            <button onClick={() => setShowNewCaseModal(true)} className="px-5 py-2.5 rounded-[20px] text-[13px] font-semibold transition-all hover:scale-105 flex items-center gap-2" style={{ background: `${P.violet}20`, color: P.text1, border: `1px solid ${P.violet}40` }}>
      ...
      <Reveal delay={100}>
        {currentCase && (
          <div className="glass-panel p-6 rounded-[28px] flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden group">
            ...
            <div className="space-y-3 relative z-10">
              ...
              <h2 className="text-xl font-bold text-[var(--color-text1)] leading-tight">{currentCase.title}</h2>
            ...
            <div className="flex items-center gap-6 text-[12px] font-mono border-t md:border-t-0 md:border-l pt-4 md:pt-0 md:pl-8 shrink-0 relative z-10" style={{ borderColor: P.border, color: P.text2 }}>
              <div>
                ...
                <span className="font-semibold text-[var(--color-text1)] flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" style={{ color: P.blue }} /> {currentCase.location}</span>
              </div>
              <div>
                <span className="text-[10px] block mb-1" style={{ color: P.text3 }}>OFFICER</span>
                <span className="font-semibold text-[var(--color-text1)] flex items-center gap-1.5"><User className="w-3.5 h-3.5" style={{ color: P.violet }} /> {currentCase.assigned_officer}</span>
              </div>
            </div>
          </div>
        )}
      ...
      <Reveal delay={150}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4" style={{ borderBottom: `1px solid ${P.border}` }}>
          ...
          {activeTab === "timeline" && (
            <div className="flex items-center gap-2">
              <button onClick={() => setShowAddForm(!showAddForm)} className="px-4 py-2.5 rounded-[20px] text-[12px] font-bold text-[var(--color-text1)] flex items-center gap-2 transition-all hover:scale-105" style={{ background: `linear-gradient(135deg, ${P.blue}, #3B82F6)` }}>
                <Plus className="w-4 h-4" /> Add Log Marker
              </button>
              <button onClick={() => { setDatasetForm({ dataset_name: `${selectedCaseId} Dataset - ${new Date().toLocaleDateString()}`, description: `Compiled evidence logs for ${selectedCaseId}` }); setShowSaveDatasetModal(true) }} className="px-4 py-2.5 rounded-[20px] text-[12px] font-bold text-[var(--color-text1)] flex items-center gap-2 transition-all hover:scale-105" style={{ background: `linear-gradient(135deg, ${P.coral}, ${P.coralSoft})` }}>
              ...
              <button onClick={handleExportJSON} className="p-2.5 rounded-[20px] transition-all hover:scale-105 glass-panel" title="Export as JSON">
                <Download className="w-4 h-4 text-[var(--color-text1)]" />
              </button>
            </div>
          )}
        </div>
          ...
          <form onSubmit={handleAddNode} className="glass-panel p-6 rounded-[28px] border space-y-5" style={{ borderColor: `${P.blue}30` }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 text-[12px]">
              <div>
                ...
                <input type="text" placeholder="e.g. CCTV Footage Captured" value={newNode.title} onChange={(e) => setNewNode({ ...newNode, title: e.target.value })} className="w-full p-3 rounded-[20px] text-[var(--color-text1)] outline-none" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
              </div>
              <div>
                <label className="font-semibold block mb-1.5" style={{ color: P.text2 }}>Event Timestamp</label>
                <input type="text" placeholder="YYYY-MM-DD HH:MM" value={newNode.event_timestamp} onChange={(e) => setNewNode({ ...newNode, event_timestamp: e.target.value })} className="w-full p-3 rounded-[20px] text-[var(--color-text1)] font-mono outline-none" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
              </div>
              <div>
                <label className="font-semibold block mb-1.5" style={{ color: P.text2 }}>Evidence Category</label>
                <select value={newNode.evidence_type} onChange={(e) => setNewNode({ ...newNode, evidence_type: e.target.value })} className="w-full p-3 rounded-[20px] text-[var(--color-text1)] outline-none cursor-pointer" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }}>
                  ...
                  <option value="ANPR_LOG" style={{ background: "#09090B" }}>🚘 ANPR License Plate Scan</option>
                  <option value="STATEMENT" style={{ background: "#09090B" }}>📝 Witness Statement</option>
                </select>
              </div>
            </div>
            <div>
              <label className="font-semibold block mb-1.5 text-[12px]" style={{ color: P.text2 }}>Detailed Summary & Forensic Note</label>
              <textarea placeholder="Enter detailed observation notes..." value={newNode.description} onChange={(e) => setNewNode({ ...newNode, description: e.target.value })} className="w-full p-3 rounded-[20px] text-[var(--color-text1)] text-[13px] h-24 resize-none outline-none" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
            </div>
            <div className="flex justify-end gap-3">
              ...
              <button type="submit" className="px-5 py-2.5 rounded-[20px] text-[12px] font-bold text-[var(--color-text1)] transition-all hover:scale-105" style={{ background: `linear-gradient(135deg, ${P.blue}, #3B82F6)` }}>Commit Node</button>
            </div>
          </form>
        </Reveal>
      )}
          ...
          {timeline.length === 0 ? (
            <Reveal>
              <div className="glass-panel p-12 text-center rounded-[28px]">
                ...
                <h3 className="text-base font-bold text-[var(--color-text1)] mb-1">No Evidence Logs Recorded Yet</h3>
                <p className="text-[13px] text-[#A1A1AA] max-w-sm mx-auto">Click "Add Log Marker" above to begin building the timeline.</p>
              </div>
            </Reveal>
          ) : (
            <div className="relative pl-6 md:pl-8 space-y-8 before:absolute before:left-3 md:before:left-4 before:top-2 before:bottom-2 before:w-px before:bg-gradient-to-b before:from-[#8B5CF6] before:via-[#3B82F6] before:to-transparent">
              {timeline.map((node, i) => (
                <Reveal key={node.id || i} delay={i * 80}>
                  <div className="relative group">
                    <div className="absolute -left-6 md:-left-8 top-1.5 w-6 h-6 rounded-full bg-[#09090B] border-2 flex items-center justify-center transition-all group-hover:scale-110 group-hover:shadow-[0_0_12px_rgba(139,92,246,0.5)]" style={{ borderColor: P.violet }}>
                      <div className="w-1.5 h-1.5 rounded-full animate-ping" style={{ background: P.violet }} />
                    </div>
                    <div className="glass-panel p-6 rounded-[28px] hover:-translate-y-1 transition-all duration-300">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 mb-3" style={{ borderBottom: `1px solid ${P.border}` }}>
                        <div className="flex items-center gap-3">
                          <div className="p-2 rounded-[20px]" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }}>
                            {getEvidenceIcon(node.evidence_type)}
                          </div>
                          <div>
                            <h3 className="text-[15px] font-bold text-[var(--color-text1)] leading-tight">{node.title}</h3>
      ...
      {activeTab === "datasets" && (
        <div className="space-y-6">
          <Reveal>
            <div>
              <h3 className="text-[16px] font-bold text-[var(--color-text1)] flex items-center gap-2 mb-1">
          ...
          {datasets.length === 0 ? (
            <Reveal delay={100}>
              <div className="glass-panel p-12 text-center rounded-[28px]">
                ...
                <h3 className="text-base font-bold text-[var(--color-text1)] mb-1">No Stored Case Datasets Yet</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {datasets.map((ds, i) => (
                <Reveal key={ds.id} delay={i * 80}>
                  <div className="glass-panel p-6 rounded-[28px] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between h-full relative group overflow-hidden">
                    ...
                    <div className="space-y-3 relative z-10">
                      ...
                      <h4 className="text-[15px] font-bold text-[var(--color-text1)]">{ds.dataset_name}</h4>
          ...
          <form onSubmit={handleCreateCase} className="glass-panel-heavy p-8 rounded-3xl max-w-lg w-full space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4 text-[12px]">
              <div>
                ...
                <input type="text" value={newCase.case_id} onChange={(e) => setNewCase({ ...newCase, case_id: e.target.value })} className="w-full p-3 rounded-[20px] font-mono outline-none text-[var(--color-text1)]" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
              </div>
              <div>
                <label className="font-semibold block mb-1.5" style={{ color: P.text2 }}>Category</label>
                <select value={newCase.category} onChange={(e) => setNewCase({ ...newCase, category: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-[var(--color-text1)] cursor-pointer" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }}>
                  ...
                  <option value="Traffic Negligence" style={{ background: "#09090B" }}>Traffic Negligence</option>
                </select>
              </div>
            </div>
            <div>
              <label className="font-semibold block mb-1.5 text-[12px]" style={{ color: P.text2 }}>Case Title</label>
              <input type="text" placeholder="e.g. Brigade Road Jewelry Store Incident" value={newCase.title} onChange={(e) => setNewCase({ ...newCase, title: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-[var(--color-text1)] text-[13px]" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
            </div>
            <div className="grid grid-cols-2 gap-4 text-[12px]">
              <div>
                <label className="font-semibold block mb-1.5" style={{ color: P.text2 }}>Priority</label>
                <select value={newCase.priority} onChange={(e) => setNewCase({ ...newCase, priority: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-[var(--color-text1)] cursor-pointer" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }}>
                  ...
                  <option value="LOW" style={{ background: "#09090B" }}>LOW</option>
                </select>
              </div>
              <div>
                <label className="font-semibold block mb-1.5" style={{ color: P.text2 }}>Location / Division</label>
                <input type="text" value={newCase.location} onChange={(e) => setNewCase({ ...newCase, location: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-[var(--color-text1)]" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
              </div>
            </div>
            <div>
              <label className="font-semibold block mb-1.5 text-[12px]" style={{ color: P.text2 }}>Case Brief</label>
              <textarea placeholder="Initial incident description..." value={newCase.summary} onChange={(e) => setNewCase({ ...newCase, summary: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-[var(--color-text1)] text-[13px] h-20 resize-none" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
            </div>
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" className="px-5 py-2.5 rounded-[20px] text-[12px] font-bold text-[var(--color-text1)] transition-all hover:scale-105" style={{ background: `linear-gradient(135deg, ${P.violet}, ${P.violetDim})` }}>Create Case</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200" style={{ background: "rgba(10,1,24,0.85)", backdropFilter: "blur(12px)" }}>
          <form onSubmit={handleSaveDataset} className="glass-panel-heavy p-8 rounded-3xl max-w-md w-full space-y-5">
            ...
            <div>
              ...
              <input type="text" value={datasetForm.dataset_name} onChange={(e) => setDatasetForm({ ...datasetForm, dataset_name: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-[var(--color-text1)] text-[13px] font-bold" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
            </div>
            <div>
              <label className="font-semibold block mb-1.5 text-[12px]" style={{ color: P.text2 }}>Description</label>
              <textarea value={datasetForm.description} onChange={(e) => setDatasetForm({ ...datasetForm, description: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-[var(--color-text1)] text-[13px] h-20 resize-none" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} />
            </div>
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" className="px-5 py-2.5 rounded-[20px] text-[12px] font-bold text-[var(--color-text1)] transition-all hover:scale-105" style={{ background: `linear-gradient(135deg, ${P.coral}, ${P.coralSoft})` }}>Store Dataset</button>
            </div>
          </form>
        </div>
      )}
