    ...
    <div className="max-w-[1320px] mx-auto space-y-6 pb-20">
      ...
      <Reveal>
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6" style={{ borderBottom: `1px solid ${P.border}` }}>
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground leading-tight">Case Management & Timelines</h1>
        ...
        {currentCase && (
          <div className="glass-panel p-6 rounded-[28px] flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden group">
            ...
            <div className="space-y-3 relative z-10">
              ...
              <h2 className="text-xl font-bold text-foreground leading-tight">{currentCase.title}</h2>
            ...
            <div className="flex items-center gap-6 text-[12px] font-mono border-t md:border-t-0 md:border-l pt-4 md:pt-0 md:pl-8 shrink-0 relative z-10" style={{ borderColor: P.border, color: P.text2 }}>
              <div>
                ...
                <span className="font-semibold text-foreground flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" style={{ color: P.blue }} /> {currentCase.location}</span>
              </div>
              <div>
                <span className="text-[10px] block mb-1" style={{ color: P.text3 }}>OFFICER</span>
                <span className="font-semibold text-foreground flex items-center gap-1.5"><User className="w-3.5 h-3.5" style={{ color: P.violet }} /> {currentCase.assigned_officer}</span>
              </div>
            </div>
          </div>
        )}
      ...
      <Reveal delay={150}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4" style={{ borderBottom: `1px solid ${P.border}` }}>
          ...
          {activeTab === "timeline" && (
            <div className="flex items-center gap-2">
              <button onClick={() => setShowAddForm(!showAddForm)} className="px-4 py-2.5 rounded-[20px] text-[12px] font-bold text-foreground flex items-center gap-2 transition-all hover:scale-105" style={{ background: `linear-gradient(135deg, ${P.blue}, #3B82F6)` }}>
                <Plus className="w-4 h-4" /> Add Log Marker
              </button>
              <button onClick={() => { setDatasetForm({ dataset_name: `${selectedCaseId} Dataset - ${new Date().toLocaleDateString()}`, description: `Compiled evidence logs for ${selectedCaseId}` }); setShowSaveDatasetModal(true) }} className="px-4 py-2.5 rounded-[20px] text-[12px] font-bold text-foreground flex items-center gap-2 transition-all hover:scale-105" style={{ background: `linear-gradient(135deg, ${P.coral}, ${P.coralSoft})` }}>
          ...
          <form onSubmit={handleAddNode} className="glass-panel p-6 rounded-[28px] border space-y-5" style={{ borderColor: `${P.blue}30` }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 text-[12px]">
              <div>
                ...
                <input type="text" placeholder="e.g. CCTV Footage Captured" value={newNode.title} onChange={(e) => setNewNode({ ...newNode, title: e.target.value })} className="w-full p-3 rounded-[20px] text-foreground outline-none" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
              </div>
              <div>
                <label className="font-semibold block mb-1.5" style={{ color: P.text2 }}>Event Timestamp</label>
                <input type="text" placeholder="YYYY-MM-DD HH:MM" value={newNode.event_timestamp} onChange={(e) => setNewNode({ ...newNode, event_timestamp: e.target.value })} className="w-full p-3 rounded-[20px] text-foreground font-mono outline-none" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
              </div>
              <div>
                <label className="font-semibold block mb-1.5" style={{ color: P.text2 }}>Evidence Category</label>
                <select value={newNode.evidence_type} onChange={(e) => setNewNode({ ...newNode, evidence_type: e.target.value })} className="w-full p-3 rounded-[20px] text-foreground outline-none cursor-pointer" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }}>
            ...
            <div>
              ...
              <textarea placeholder="Enter detailed observation notes..." value={newNode.description} onChange={(e) => setNewNode({ ...newNode, description: e.target.value })} className="w-full p-3 rounded-[20px] text-foreground text-[13px] h-24 resize-none outline-none" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
            </div>
            <div className="flex justify-end gap-3">
              ...
              <button type="submit" className="px-5 py-2.5 rounded-[20px] text-[12px] font-bold text-foreground transition-all hover:scale-105" style={{ background: `linear-gradient(135deg, ${P.blue}, #3B82F6)` }}>Commit Node</button>
            </div>
          </form>
        </Reveal>
      )}
          ...
          {timeline.length === 0 ? (
            <Reveal>
              <div className="glass-panel p-12 text-center rounded-[28px]">
                ...
                <h3 className="text-base font-bold text-foreground mb-1">No Evidence Logs Recorded Yet</h3>
                    ...
                    <div className="glass-panel p-6 rounded-[28px] hover:-translate-y-1 transition-all duration-300">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 mb-3" style={{ borderBottom: `1px solid ${P.border}` }}>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h3 className="text-[15px] font-bold text-foreground leading-tight">{node.title}</h3>
      ...
      {activeTab === "datasets" && (
        <div className="space-y-6">
          <Reveal>
            <div>
              <h3 className="text-[16px] font-bold text-foreground flex items-center gap-2 mb-1">
          ...
          {datasets.length === 0 ? (
            <Reveal delay={100}>
              <div className="glass-panel p-12 text-center rounded-[28px]">
                ...
                <h3 className="text-base font-bold text-foreground mb-1">No Stored Case Datasets Yet</h3>
                ...
                <Reveal key={ds.id} delay={i * 80}>
                  <div className="glass-panel p-6 rounded-[28px] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between h-full relative group overflow-hidden">
                    ...
                    <div className="space-y-3 relative z-10">
                      ...
                      <h4 className="text-[15px] font-bold text-foreground">{ds.dataset_name}</h4>
          ...
          <form onSubmit={handleCreateCase} className="glass-panel-heavy p-8 rounded-3xl max-w-lg w-full space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4 text-[12px]">
              <div>
                ...
                <input type="text" value={newCase.case_id} onChange={(e) => setNewCase({ ...newCase, case_id: e.target.value })} className="w-full p-3 rounded-[20px] font-mono outline-none text-foreground" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
              </div>
              <div>
                <label className="font-semibold block mb-1.5" style={{ color: P.text2 }}>Category</label>
                <select value={newCase.category} onChange={(e) => setNewCase({ ...newCase, category: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-foreground cursor-pointer" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }}>
            ...
            <div>
              ...
              <input type="text" placeholder="e.g. Brigade Road Jewelry Store Incident" value={newCase.title} onChange={(e) => setNewCase({ ...newCase, title: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-foreground text-[13px]" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
            </div>
            <div className="grid grid-cols-2 gap-4 text-[12px]">
              <div>
                <label className="font-semibold block mb-1.5" style={{ color: P.text2 }}>Priority</label>
                <select value={newCase.priority} onChange={(e) => setNewCase({ ...newCase, priority: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-foreground cursor-pointer" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }}>
              ...
              <div>
                ...
                <input type="text" value={newCase.location} onChange={(e) => setNewCase({ ...newCase, location: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-foreground" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
              </div>
            </div>
            <div>
              <label className="font-semibold block mb-1.5 text-[12px]" style={{ color: P.text2 }}>Case Brief</label>
              <textarea placeholder="Initial incident description..." value={newCase.summary} onChange={(e) => setNewCase({ ...newCase, summary: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-foreground text-[13px] h-20 resize-none" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
            </div>
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" className="px-5 py-2.5 rounded-[20px] text-[12px] font-bold text-foreground transition-all hover:scale-105" style={{ background: `linear-gradient(135deg, ${P.violet}, ${P.violetDim})` }}>Create Case</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200" style={{ background: "rgba(10,1,24,0.85)", backdropFilter: "blur(12px)" }}>
          <form onSubmit={handleSaveDataset} className="glass-panel-heavy p-8 rounded-3xl max-w-md w-full space-y-5">
            ...
            <div>
              ...
              <input type="text" value={datasetForm.dataset_name} onChange={(e) => setDatasetForm({ ...datasetForm, dataset_name: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-foreground text-[13px] font-bold" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} required />
            </div>
            <div>
              <label className="font-semibold block mb-1.5 text-[12px]" style={{ color: P.text2 }}>Description</label>
              <textarea value={datasetForm.description} onChange={(e) => setDatasetForm({ ...datasetForm, description: e.target.value })} className="w-full p-3 rounded-[20px] outline-none text-foreground text-[13px] h-20 resize-none" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${P.border}` }} />
            </div>
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" className="px-5 py-2.5 rounded-[20px] text-[12px] font-bold text-foreground transition-all hover:scale-105" style={{ background: `linear-gradient(135deg, ${P.coral}, ${P.coralSoft})` }}>Store Dataset</button>
            </div>
          </form>
        </div>
      )}
