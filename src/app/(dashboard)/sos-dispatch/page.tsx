...
export default function SOSDispatchPage() {
  ...
  const fetchAlerts = async () => {
    try {
      const res = await fetch("https://kaaval-backend-50044365217.development.catalystappsail.in/api/sos/alert")
  ...
  const triggerSOS = async (incidentType = "CRITICAL EMERGENCY SOS", priority = "CRITICAL") => {
    ...
    try {
      const res = await fetch("https://kaaval-backend-50044365217.development.catalystappsail.in/api/sos/alert", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ caller_name: "Field Officer (Instant Trigger)", caller_phone: "+91 9876543210", latitude: lat, longitude: lng, address: addr, incident_type: incidentType, priority: priority }) })
      if (res.ok) {
        fetchAlerts()
  ...
  const handleUpdateStatus = async (alertId?: number, newStatus = "RESOLVED") => {
    if (!alertId) return
    try {
      await fetch("https://kaaval-backend-50044365217.development.catalystappsail.in/api/sos/alert", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ alert_id: alertId, status: newStatus }) })
      fetchAlerts()
      ...
      <Reveal delay={100}>
        <div className="glass-panel p-8 sm:p-12 rounded-3xl flex flex-col items-center justify-center text-center relative overflow-hidden border" style={{ borderColor: `${P.red}30`, background: "rgba(10,1,24,0.4)" }}>
          ...
          <div className="max-w-md mx-auto space-y-2 relative z-10 mb-8">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-t1">INSTANT SOS SIGNAL</h2>
          ...
          <button
            ...
            className="relative group w-48 h-48 mx-auto rounded-full border-4 shadow-2xl transition-all duration-300 transform active:scale-95 flex flex-col items-center justify-center gap-2 text-t1 font-black tracking-wider z-10 hover:scale-105"
      ...
      <Reveal delay={150}>
        <div className="space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-4" style={{ borderColor: P.border }}>
            <h2 className="text-[16px] font-bold text-t1 flex items-center gap-2">
                ...
                <div className="glass-panel p-6 rounded-3xl flex flex-col h-full border relative overflow-hidden group hover:-translate-y-1 transition-all duration-300" style={{ borderColor: P.border, background: P.surface }}>
                  ...
                  <div className="flex items-center justify-between border-b pb-4 mb-4 relative z-10" style={{ borderColor: P.border }}>
                    <div>
                      ...
                      <h3 className="text-[16px] font-extrabold text-t1 leading-tight">{alert.incident_type}</h3>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-[12px] relative z-10 flex-1 content-start mb-5">
                    <div className="p-3.5 rounded-[28px] border flex flex-col justify-center" style={{ background: "rgba(0,0,0,0.2)", borderColor: P.border }}>
                      ...
                      <p className="font-bold text-t1 text-[13px] truncate">{alert.caller_name}</p>
                    </div>
                    <div className="p-3.5 rounded-[28px] border flex flex-col justify-center" style={{ background: "rgba(0,0,0,0.2)", borderColor: P.border }}>
                  ...
                  <div className="p-4 rounded-[28px] flex items-center justify-between text-[11px] relative z-10" style={{ background: "rgba(0,0,0,0.4)", border: `1px solid ${P.border}` }}>
                    ...
                    {alert.status !== 'RESOLVED' ? (
                      <button onClick={() => handleUpdateStatus(alert.id, 'RESOLVED')} className="px-4 py-2 text-t1 font-bold rounded-[20px] text-[10px] uppercase tracking-wider transition-all hover:scale-105 shadow-lg" style={{ background: `linear-gradient(135deg, ${P.green}, #059669)` }}>
                        Mark Resolved
                      </button>
                    ) : (
