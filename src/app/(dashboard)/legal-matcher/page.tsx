  ...
  const handleMatch = async (inputText?: string) => {
    ...
    try {
      ...
      if (res.ok) { 
        ...
        toast.success(`Found ${data.matched_sections?.length || 0} legal matches`)
      } else throw new Error()
    } catch (e) {
      toast.info("Offline mode: Showing simulated legal matches")
      ...
      <Reveal>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6" style={{ borderBottom: `1px solid ${P.border}` }}>
          <div className="flex items-start gap-4">
            <div className="p-3 rounded-[28px] relative group overflow-hidden" style={{ background: `${P.blue}15`, border: `1px solid ${P.blue}30` }}>
        ...
        <div className="glass-panel p-8 rounded-3xl border flex flex-col xl:flex-row gap-8 relative overflow-hidden" style={{ borderColor: P.border, background: P.surface }}>
          ...
          <div className="flex-1 flex flex-col space-y-5">
            <div className="flex items-center justify-between relative z-10">
              <label className="text-[14px] font-bold text-[var(--color-text1)] flex items-center gap-2">
              ...
              {sampleScenarios.map((sc, i) => (
                <button key={i} onClick={() => { setStatement(sc.text); handleMatch(sc.text) }} className="px-4 py-3 rounded-[20px] text-left transition-all hover:translate-x-1 group flex items-center justify-between" style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${P.border}` }}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12px] font-medium text-[var(--color-text1)] group-hover:text-blue-400 transition-colors">{sc.label}</span>
                  </div>
      ...
      {searched && (
        <Reveal delay={150}>
          <div className="space-y-5">
            <h2 className="text-[16px] font-bold text-[var(--color-text1)] flex items-center gap-2 border-b pb-4" style={{ borderColor: P.border }}>
                    ...
                    <div className="glass-panel p-8 rounded-3xl border flex flex-col relative overflow-hidden group hover:-translate-y-1 transition-all duration-300 shadow-xl" style={{ borderColor: P.border, background: P.surface }}>
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-5 mb-5 relative z-10" style={{ borderColor: P.border }}>
                        <div>
                          ...
                          <h3 className="text-xl sm:text-2xl font-extrabold text-[var(--color-text1)] leading-tight">{res.offense_title}</h3>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          ...
                          <span className="px-4 py-2 rounded-full font-mono text-[11px] font-medium" style={{ background: "#09090B", color: P.text3, border: `1px solid ${P.border}` }}>Legacy: {res.ipc_section}</span>
                        </div>
                      </div>
                      ...
                      <div className="mt-5 p-5 rounded-[28px] border flex items-start gap-4 relative z-10" style={{ background: `${P.blue}10`, borderColor: `${P.blue}20` }}>
                        ...
                        <div>
                          ...
                          <p className="text-[14px] font-medium text-[var(--color-text1)] leading-relaxed">{res.mandatory_action}</p>
                        </div>
                      </div>
                    </div>
                  </Reveal>
